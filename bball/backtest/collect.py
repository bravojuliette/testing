...
def collect_day(client: ApiClient, conn, league_id: int, day: str, use_cache: bool = True) -> dict:
    ...
    for e in events:
        ...
        # UTC explicito -- fromtimestamp() sin tz usa la zona horaria local del
        # proceso, que en GitHub Actions es UTC pero en cualquier otro entorno
        # puede no serlo. bball_picks (live/scan.py) ya calcula esta misma
        # fecha en UTC; con date local aqui, un partido cerca de medianoche UTC
        # podia terminar en dias distintos segun donde corriera, descuadrando
        # el corte busqueda/reserva de sweep.py (compara p.date como string).
        game_date = date.fromtimestamp(ts, tz=timezone.utc).isoformat() if ts else day[:4] + "-" + day[4:6] + "-" + day[6:8]
        conn.execute(
