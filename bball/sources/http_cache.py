"""Cliente HTTP con cache persistente (tabla bball_http_cache, separada de la
de tt_elite) y rate-limiting para BetsAPI. Copia deliberada del patron de
tt_elite/sources/http_cache.py -- mismo problema (no repetir llamadas ya
hechas, no pasarse del rate-limit), pero sin importar del paquete tt_elite/
para mantener los dos sistemas totalmente desacoplados.
"""
...
USER_AGENT = "BBallTotalsApp/1.0"


class ApiClient:
    def __init__(self, conn, betsapi_token: str, min_interval: float = 1.05):
        self.conn = conn
    ...
    def get_json(self, url: str, params: dict | None = None, *, prefix: str = "http",
        ...
        if use_cache:
            row = self.conn.execute(
                "SELECT body FROM bball_http_cache WHERE cache_key = ?", (key,)
            ).fetchone()
            if row:
        ...
        for attempt in range(max_retries):
            ...
            if use_cache:
                self.conn.execute(
                    "INSERT OR REPLACE INTO bball_http_cache(cache_key, prefix, fetched_at, body) VALUES (?, ?, ?, ?)",
    ...
    def bets(self, path: str, params: dict, *, prefix: str, use_cache: bool = True) -> dict:
        if not self.token:
            raise RuntimeError("Falta BETSAPI_TOKEN_BBALL (variable de entorno / secret).")
        q = dict(params)
        q["token"] = self.token
